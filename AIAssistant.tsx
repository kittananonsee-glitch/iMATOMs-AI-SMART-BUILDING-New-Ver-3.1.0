  ...
  const sendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: userMsg,
        config: {
          systemInstruction: `You are the iMATOMs SMART BUILDING PRO-Ver 3.1.0 AI ANALYTICS. You assist hospital engineers and staff. Current system context: ${systemContext}. Use grounding if needed for technical standards.`,
          tools: [{ googleSearch: {} }]
        }
      });
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: {
          ...
          systemInstruction: `You are the iMATOMs Ver 3.1.0 AI Voice Assistant. Respond concisely. Current context: ${systemContext}`,
    ...
    <div className="fixed inset-y-0 right-0 w-full sm:w-96 bg-[#0a0e17] border-l border-cyan-500/20 z-[100] flex flex-col shadow-2xl animate-slideLeft">
      <div className="p-6 border-b border-cyan-500/10 flex items-center justify-between bg-black/40">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-display text-sm font-bold text-white uppercase tracking-widest">Gemini-AI Link</h2>
            <div className="flex items-center gap-1.5">
               <span className={`w-1.5 h-1.5 rounded-full ${isLiveActive ? 'bg-red-500 animate-pulse' : 'bg-emerald-500'}`}></span>
               <span className="text-[9px] text-white/40 uppercase font-black">{isLiveActive ? 'Live Voice Active' : 'Analytics Ready'}</span>
            </div>
          </div>
        </div>
      ...
      <div className="p-4 bg-black/60 border-t border-white/5">
        ...
        <p className="text-[8px] text-white/20 uppercase text-center mt-3 tracking-widest">
